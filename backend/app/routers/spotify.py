...
from backend.app.schemas import SpotifyStatus
...
@router.get("/users/{user_slug}/spotify/connect")
def connect_spotify(user_slug: str) -> RedirectResponse:
    session_factory = _session_factory()
    with session_factory() as session:
        try:
            UserRepository(session).require_user_by_slug(user_slug)
            return RedirectResponse(spotify_oauth_service.authorize_url(user_slug))
        except (KeyError, LookupError) as exc:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))


@router.get("/users/{user_slug}/spotify/status", response_model=SpotifyStatus)
def spotify_status(user_slug: str) -> SpotifyStatus:
    session_factory = _session_factory()
    with session_factory() as session:
        try:
...
def sync_spotify_now(user_slug: str) -> dict[str, int]:
    try:
